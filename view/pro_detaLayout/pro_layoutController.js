  ...
  .controller('pro_layoutController',['$scope','buymodal','$location','$state','$stateParams','$ionicPopup','API','$rootScope',function($scope,buymodal,$location,$state,$stateParams,$ionicPopup,API,$rootScope){
        ...
        //向后台发送请求,获取后台数据接口
          ...
          .then(function(data){
              ...
              $scope.pro_layout.pid = data.data[0].pid;
              ...
              $scope.pro_layout.product_class=[];
                 ...
                 console.log($scope.pro_layout.product_class);
                  if( $scope.pro_layout.product_class.length >1){
                      for(let i=0;i<$scope.pro_layout.product_class.length;i++){
      ...
      $scope.join=function(){
        $scope.join_shopping = {
            ...
            uid:2,
            pid:$scope.pro_layout.pid
        }
        let aID=$scope.pro_layout.selected;
        let selected = $scope.pro_layout.selected;
         if($scope.pro_layout.quan > 0 && selected[0] != '' && selected[1] != '' && selected[2] != '' && selected[3] != ''){
          API.fetchGet('http://127.0.0.1:9000/join_shopping',$scope.join_shopping)
           ...
           if( selected[0] == '' || selected[1] == '' || selected[2] == '' || selected[3] == ''){
             for(let i=0;i<$scope.pro_layout.selected.length;i++){
                if($scope.pro_layout.selected[i] == ''){
                    $rootScope.prompt_box('请选择' + $scope.pro_layout.class_title[i]);
                    return false
                }
              }
          }else if($scope.pro_layout.quan == 0){
      ...
      $scope.buy=function(){
        let selected = $scope.pro_layout.selected;
        let buy_obj={};
        if(selected[0] != '' && selected[1] != '' && selected[2] != '' && selected[3] != '' && $scope.pro_layout.quan > 0){
             buy_obj.txt=selected;
             ...
             buy_obj.pid=$scope.pro_layout.pid;
             buymodal.closeModal($scope);
        }else{
          if( selected[0] == '' || selected[1] == '' || selected[2] == '' || selected[3] == ''){
             for(let i=0;i<$scope.pro_layout.selected.length;i++){
